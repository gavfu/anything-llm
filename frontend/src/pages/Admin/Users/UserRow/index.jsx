...
export default function UserRow({ currUser, user }) {
  ...
  const handleSuspend = async () => {
    if (
      !window.confirm(
        `Are you sure you want to suspend ${user.username}?\nAfter you do this they will be logged out and unable to log back into this instance of PlexusAI until unsuspended by an admin.`
      )
    )
      return false;
  ...
  const handleDelete = async () => {
    if (
      !window.confirm(
        `Are you sure you want to delete ${user.username}?\nAfter you do this they will be logged out and unable to use this instance of PlexusAI.\n\nThis action is irreversible.`
      )
    )
      return false;
