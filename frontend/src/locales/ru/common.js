...
const TRANSLATIONS = {
  onboarding: {
    ...
    llm: {
      ...
      description:
        "PlexusAI может работать с различными провайдерами LLM. Этот сервис будет обеспечивать обработку чата.",
    },
    userSetup: {
    ...
    survey: {
      title: "Добро пожаловать в PlexusAI",
      description:
        "Помогите нам сделать PlexusAI, созданным с учётом ваших потребностей (необязательно).",
      email: "Какой у вас адрес электронной почты?",
      useCase: "Для чего вы будете использовать PlexusAI?",
      useCaseWork: "Для работы",
      useCasePersonal: "Для личного использования",
      useCaseOther: "Другое",
      comment: "Откуда вы узнали о PlexusAI?",
      commentPlaceholder:
    ...
    workspace: {
      ...
      description:
        "Создайте ваше первое рабочее пространство и начните работу с PlexusAI.",
    },
  },
  common: {
  ...
  welcomeMessage: {
    part1:
      "Добро пожаловать в PlexusAI, открытый инструмент искусственного интеллекта от Mintplex Labs, который превращает что угодно в обученный чат-бот, с которым вы можете общаться и задавать вопросы. PlexusAI - это ПО BYOK (принеси свои собственные ключи), поэтому за использование этого ПО нет подписки, платы или других сборов, кроме тех, что вы хотите использовать.",
    part2:
      "PlexusAI - это самый простой способ объединить мощные продукты ИИ, такие как OpenAi, GPT-4, LangChain, PineconeDB, ChromaDB и другие сервисы, в аккуратный пакет без лишних хлопот, чтобы повысить вашу продуктивность в 100 раз.",
    part3:
      "PlexusAI может работать полностью локально на вашем компьютере с минимальной нагрузкой, вы даже не заметите его присутствия! GPU не требуется. Также доступна установка в облаке и на локальных серверах.\nЭкосистема инструментов ИИ становится мощнее с каждым днем. PlexusAI упрощает их использование.",
  ...
  api: {
    title: "API ключи",
    description:
      "API ключи позволяют владельцу программно получать доступ к этому экземпляру PlexusAI и управлять им.",
  ...
  llm: {
    ...
    description:
      "Это учетные данные и настройки для вашего предпочтительного поставщика чата и встраивания LLM. Важно, чтобы эти ключи были актуальными и правильными, иначе PlexusAI не будет работать должным образом.",
  ...
  transcription: {
    ...
    "warn-start":
      "Использование локальной модели whisper на машинах с ограниченной оперативной памятью или процессором может привести к зависанию PlexusAI при обработке медиафайлов.",
    "warn-recommend":
  ...
  embedding: {
    ...
    "desc-end":
      "Встраивание - это процесс превращения текста в векторы. Эти учетные данные необходимы для превращения ваших файлов и подсказок в формат, который PlexusAI может использовать для обработки.",
    provider: {
      title: "Поставщик встраивания",
      description:
        "Нет необходимости в настройке при использовании встроенного механизма встраивания PlexusAI.",
    },
  },
  text: {
  ...
  vector: {
    ...
    description:
      "Это учетные данные и настройки для того, как будет функционировать ваш экземпляр PlexusAI. Важно, чтобы эти ключи были актуальными и правильными.",
    provider: {
  ...
  multi: {
    ...
    password: {
      ...
      description:
        "Защитите ваш экземпляр PlexusAI паролем. Если вы забудете его, метода восстановления не существует, поэтому убедитесь, что вы сохранили этот пароль.",
    },
    instance: {
  ...
  privacy: {
    ...
    description:
      "Это ваша конфигурация для того, как подключенные сторонние поставщики и PlexusAI обрабатывают ваши данные.",
    llm: "Выбор LLM",
  ...
  connectors: {
    ...
    upload: {
      ...
      "privacy-notice":
        "Эти файлы будут загружены в процессор документов на этом экземпляре PlexusAI. Файлы не отправляются и не передаются третьим лицам.",
    },
    pinning: {
      what_pinning: "Что такое закрепление документа?",
      pin_explained_block1:
        "Когда вы <b>закрепляете</b> документ в PlexusAI, мы вставляем всё его содержимое в окно запроса, чтобы LLM мог полностью его понять.",
      pin_explained_block2:
        "Это работает лучше всего с <b>моделями с большим контекстом</b> или небольшими файлами, критичными для базы знаний.",
      pin_explained_block3:
        "Если по умолчанию ответы PlexusAI вас не удовлетворяют, закрепление — отличный способ получить более качественные ответы одним кликом.",
      accept: "Хорошо, понял",
    },
    watching: {
      what_watching: "Что делает функция наблюдения за документом?",
      watch_explained_block1:
        "Когда вы <b>наблюдаете</b> за документом в PlexusAI, мы <i>автоматически</i> синхронизируем его содержимое с оригинальным источником через регулярные интервалы. Это автоматически обновляет содержимое во всех рабочих пространствах, где используется этот файл.",
