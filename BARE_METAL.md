# Run PlexusAI in production without Docker
...
Here you can find the scripts and known working process to run PlexusAI outside of a Docker container.
...
PlexusAI is comprised of three main sections. The `frontend`, `server`, and `collector`. When running in production you will be running `server` and `collector` on two different processes, with a build step for compilation of the frontend.
...
PlexusAI should now be running on `http://localhost:3001`!

## Updating PlexusAI

To update PlexusAI with future updates you can `git pull origin master` to pull in the latest code and then repeat steps 2 - 5 to deploy with all changes fully.
...
_note_ You should `pkill node` before running an update so that you are not running multiple PlexusAI processes on the same instance as this can cause conflicts.
