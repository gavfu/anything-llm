 ...
 * For example, if the tools requires `npx` then the context in which PlexusAI mains process is running will need to access npx.
 * This is typically not common in our pre-built image so may not function. But this is the case anywhere MCP is used.
 *
 * PlexusAI will take care of porting MCP servers to agent-callable functions via @agent directive.
