# Setting up `PGVector` for PlexusAI
...
> [!IMPORTANT]
> If you have an existing table that you want to use as a vector database, PlexusAI **requires** that the table be
> at least minimally conform to the expected schema - this can be seen in the [index.js](./index.js) file.

_optional_ - set a table name you wish to have PlexusAI store vectors to. By default this is `anythingllm_vectors`

## Common Questions

### I cannot connect to the DB (Running PlexusAI in Docker)

If you are running PlexusAI in Docker, you will need to ensure that the DB is accessible from the container.
...
`localhost` will not work in this case as it will attempt to connect to the DB _inside the PlexusAI container_ instead of the host machine or another container.
...
Yes, you can use an existing table as a vector database. However, PlexusAI **requires** that the table be at least minimally conform to the expected schema - this can be seen in the [index.js](./index.js) file.

It is **absolutely critical** that the `embedding` column's `VECTOR(XXXX)` dimensions match the dimension of the embedder in PlexusAI. The default embedding model is 384 dimensions. However, if you are using a custom embedder, you will need to ensure that the dimension value is set correctly.

### Validate the connection to the database

When setting the connection string in or table name via the PlexusAI UI, the following validations will be attempted:
...
The embedding storage table is created by PlexusAI **on the first upsert** of a vector. If you have not yet embedding any documents, the table will not be present in the DB.

### How do I reset my vector database?

_at the workspace level in Settings > Vector Database_

You can use the "Reset Vector Database" button in the PlexusAI UI to reset your vector database. This will drop all vectors within that workspace, but the table will remain in the DB.

_reset the vector database at the db level_

For this, you will need to `DROP TABLE` from the command line or however you manage your DB. Once the table is dropped, it will be recreated by PlexusAI on the next upsert.
...
- Ensure the table has a dimension value set and this matches the dimension of the embedder in PlexusAI
