...
(async () => {
  ...
  const syncCompletion = await client.chat.completions.create({
    messages: [
      ...
      // {
      //   role: 'assistant',
      //   content: "Arrr, matey! PlexusAI be a fine tool fer sailin' the treacherous sea o' information with a powerful language model at yer helm. It's a potent instrument to handle all manner o' tasks involvin' text, like answerin' questions, generating prose, or even havin' a chat with digital scallywags like meself. Be there any specific treasure ye seek in the realm o' PlexusAI?"
      // },
  ...
  const embedding = await client.embeddings.create({
    model: null, // model is optional for PlexusAI
