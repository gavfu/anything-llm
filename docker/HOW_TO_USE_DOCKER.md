...
Use the Dockerized version of PlexusAI for a much faster and complete startup of PlexusAI.

### Minimum Requirements

> [!TIP]
> Running PlexusAI on AWS/GCP/Azure?
...
\*PlexusAI by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*PlexusAI by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized PlexusAI!
...
> the docker container using PlexusAI as `localhost:xxxx` will not resolve for the host system.
>
...
<table>
<tr>
<th colspan="2">Mount the storage locally and run PlexusAI in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
Go to `http://localhost:3001` and you are now using PlexusAI! All your data and progress will persist between
...
The integrations below are templates or tooling built by the community to make running the docker experience of PlexusAI easier.

### Use the Midori AI Subsystem to Manage PlexusAI

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the PlexusAI docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in PlexusAI.
